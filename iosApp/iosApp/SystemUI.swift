...
final class SystemUI: ObservableObject {
    ...
    // Fork: internal (upstream: private) so RootComposeViewController.refreshImmersiveSystemUI
    // can re-propagate appearance to every window (persistent Home Bar fix).
    func refresh() {
